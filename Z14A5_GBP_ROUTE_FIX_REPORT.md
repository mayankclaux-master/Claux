# Z14A5 GBP Route Fix Report

**Phase:** Z14A.5 — GBP ROUTE CONTRACT FIX  
**Date:** 2025-05-13  
**Status:** COMPLETE

## Objective

Fix ONLY the current first failing build blocker in apps/web/app/api/integrations/dispatch/gbp/route.ts by restoring canonical contract compatibility.

## Root Cause

**Exact Root Cause:** The gbp/route.ts was using obsolete event payload property names that don't match the canonical EventInsert interface from the modular runtime type system.

**Details:**
- Used `event_data` instead of canonical `payload` property
- Missing required `tenant_id` field in EventInsert

**Pattern:** Same contract mismatch as dataforseo/route.ts (fixed in Z14A.4)
...
**Build Status:** gbp/route.ts no longer the build blocker
...
./app/api/integrations/dispatch/gsc/route.ts:128:7
...
**Classification:** LOCAL TYPING MISMATCH (same pattern as dataforseo and gbp)

**Reason:** The gsc/route.ts has the same issue - using `event_data` instead of canonical `payload` property.

**Impact:** Build-time error only, not runtime critical.

## Summary

- **Files Touched:** 1 (gbp/route.ts)
...
- **gbp/route.ts Status:** ✅ FIXED
- **Next Blocker:** gsc/route.ts:128 (same pattern - event_data → payload)
