# Z14A6 GSC Route Fix Report

**Phase:** Z14A.6 — GSC ROUTE CONTRACT FIX  
**Date:** 2025-05-13  
**Status:** COMPLETE

## Objective

Fix ONLY the current first failing build blocker in apps/web/app/api/integrations/dispatch/gsc/route.ts by restoring canonical contract compatibility.

## Root Cause

**Exact Root Cause:** The gsc/route.ts was using obsolete event payload property names that don't match the canonical EventInsert interface from the modular runtime type system.

**Details:**
- Used `event_data` instead of canonical `payload` property
- Missing required `tenant_id` field in EventInsert

**Pattern:** Same contract mismatch as dataforseo/route.ts (Z14A.4) and gbp/route.ts (Z14A.5)
...
**Build Status:** gsc/route.ts no longer the build blocker
...
./app/api/integrations/dispatch/openai/route.ts:117:7
...
**Classification:** LOCAL TYPING MISMATCH (same pattern as dataforseo, gbp, gsc)

**Reason:** The openai/route.ts has the same issue - using `event_data` instead of canonical `payload` property.

**Impact:** Build-time error only, not runtime critical.

## Summary

- **Files Touched:** 1 (gsc/route.ts)
...
- **gsc/route.ts Status:** ✅ FIXED
- **Next Blocker:** openai/route.ts:117 (same pattern - event_data → payload)
