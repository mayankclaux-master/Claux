/**
 * DataForSEO Dispatch Route
 * 
 * Phase Z5 - Real Execution Cutover
 * Canonical n8n bridge for DataForSEO provider execution.
 * 
 * Responsibilities:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (governance.isProviderInCooldown('dataforseo')) {
      return NextResponse.json(
    ...
    if (governance.isProviderQuarantined('dataforseo')) {
      return NextResponse.json(
    ...
    if (!result.success) {
      await governance.handleProviderFailure('dataforseo', tenantId, executionId, result.error || 'Dispatch failed');
      
      // Check failure count for escalation
      const failureCount = governance.getProviderFailureCount('dataforseo');
      if (failureCount >= 3) {
        // Activate cooldown
        const cooldownUntil = new Date(Date.now() + 5 * 60 * 1000).toISOString(); // 5 minute cooldown
        await governance.handleProviderCooldown('dataforseo', cooldownUntil, tenantId, executionId);
      }
    }
    ...
    console.error('DataForSEO dispatch error:', error);
    return NextResponse.json(
