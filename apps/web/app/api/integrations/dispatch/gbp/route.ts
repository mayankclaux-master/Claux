 ...
 * Canonical n8n bridge for GBP provider execution.
 * 
 * Responsibilities:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (governance.isProviderInCooldown('gbp')) {
      return NextResponse.json(
    ...
    if (governance.isProviderQuarantined('gbp')) {
      return NextResponse.json(
    ...
    if (!result.success) {
      await governance.handleProviderFailure('gbp', tenantId, executionId, result.error || 'Dispatch failed');
      
      // Check failure count for escalation
      const failureCount = governance.getProviderFailureCount('gbp');
      if (failureCount >= 3) {
        // Activate cooldown
        const cooldownUntil = new Date(Date.now() + 5 * 60 * 1000).toISOString(); // 5 minute cooldown
        await governance.handleProviderCooldown('gbp', cooldownUntil, tenantId, executionId);
      }
    }
    ...
    console.error('GBP dispatch error:', error);
    return NextResponse.json(
