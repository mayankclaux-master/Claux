/**
 * OpenAI Dispatch Route
 * 
 * Phase Z5 - Real Execution Cutover
 * Canonical n8n bridge for OpenAI provider execution.
 * 
 * Responsibilities:
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (governance.isProviderInCooldown('openai') || governance.isProviderQuarantined('openai')) {
    ...
    if (!result.success) {
      await governance.handleProviderFailure('openai', tenantId, executionId, result.error || 'Dispatch failed');
      
      // Check failure count for escalation
      const failureCount = governance.getProviderFailureCount('openai');
      if (failureCount >= 3) {
        // Activate cooldown
        const cooldownUntil = new Date(Date.now() + 5 * 60 * 1000).toISOString(); // 5 minute cooldown
        await governance.handleProviderCooldown('openai', cooldownUntil, tenantId, executionId);
      }
    }
    ...
    console.error('OpenAI dispatch error:', error);
    return NextResponse.json(
