...
export async function POST(request: Request) {
  ...
  try {
    ...
    await recoverStaleRuns(tenantId, "PUBLISH");

    // Idempotency check: use agent_runs as source of truth
    const activeRun = await getActiveRun(tenantId, "PUBLISH");
    ...
    const runId = await createAgentRunWithState(tenantId, "PUBLISH", userId, executionId);

    // Create context
    const context = {
      tenantId,
      agent: "PUBLISH" as const,
      runId
    };
